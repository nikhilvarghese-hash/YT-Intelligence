      ...
      {loading ? (
        ...
        <div className="flex gap-4 min-h-[520px]">
          ...
          <div className="w-72 flex-shrink-0 border border-border rounded-lg overflow-hidden flex flex-col">
            <div className="px-2 py-2 border-b border-border bg-secondary/20">
              ...
              <div className="mt-1.5 text-[10px] text-muted-foreground px-1">
                {topics.filter(t => !search || t.keyword.includes(search.toLowerCase())).length} topics · {formatNumber(totalVideos)} videos
              </div>
            </div>
            <div className="flex-1 overflow-y-auto divide-y divide-border">
              {topics.filter(t => !search || t.keyword.includes(search.toLowerCase())).length === 0 && (
                <div className="text-center py-12 text-sm text-muted-foreground px-4">
                  {search ? `No topics matching "${search}"` : 'No topics found. Import some videos first.'}
                </div>
              )}
              {topics.filter(t => !search || t.keyword.includes(search.toLowerCase())).map((t, i) => (
                <button
                  key={t.keyword}
